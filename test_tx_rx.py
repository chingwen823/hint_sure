...
def main():
    ...
    def send_ack_pkt(my_tb, temp_list):
        while temp_list:
            ...
            print "Data_list pop: pktno {}, time_data {}".format(pktno, time_data)
            pktno += 1
            print "send ack pktno {}, time_data {}".format(pktno, time_data)
            data = (pkt_size - 2 - date_len) * chr(pktno & 0xff)
    ...
    def rx_callback(ok, payload):
        ...
        foo.data_list.append((pktno, rxtime))

        # if pktno < max_pkt:
        #     foo.stop_init_pkt = True
        #
        #     # tx - send packet
        #     pktno += 1
        #     data = (pkt_size - 2 - date_len) * chr(pktno & 0xff)
    ...
    while count < 100:
        ...
        count += 1
        if foo.data_list:
            print "data_list len {}".format(len(foo.data_list))
            #threaded_send_ack_pkt(tb, foo.data_list)
            send_ack_pkt(tb, foo.data_list)

            # new_list = list(foo.data_list)
            # while new_list:
            #     pktno, time_data = foo.data_list.pop(0)
            #     print "Pop from data_list: pktno {}, time_data {}".format(pktno, time_data)
            #     pktno += 1
            #     print "send ack pktno {}, time_data {}".format(pktno, time_data)
            #
            #     #data = (pkt_size - 2) * chr(pktno & 0xff)
