...
def main():
    ...
    def send_test_pkt(my_tb, pkt_size, pkt_no,numdata):
        ...
        data_size = len(payload_prefix) + TIMESTAMP_LEN + len(testup) + len(data)
        dummy = (pkt_size - data_size) * chr(pkt_no & 0xff)
        now_timestamp = my_tb.sink.get_time_now().get_real_secs()
        now_timestamp_str = '{:.3f}'.format(now_timestamp)
    ...
    def rx_bs_callback(ok, payload):    # For BS
        ...
        if pkt_type == PacketType.VFS_PKT.index:
            ...
            return
#    def rx_rbk_node_callback(ok, payload):    # For Node
#        global down_payload
...
#                str(datetime.fromtimestamp(now_timestamp)), pktno, str(datetime.fromtimestamp(pkt_timestamp)), mean_delta))
    ...
    def fire_at_absolute_second():
        for i in range(10000):
            check_time = rx_tb.sink.get_time_now().get_real_secs()
    ...
    def usrp_sleep(interval_sec):
        wake_up_timestamp = rx_tb.sink.get_time_now().get_real_secs() + interval_sec
        for i in range(50000):
            now_timestamp = rx_tb.sink.get_time_now().get_real_secs()
    ...
    def check_thread_is_done(max_pkt_amt):
        for i in range(10000):
            if not my_thread.is_alive() and my_iteration >= max_pkt_amt:
                now_ts = rx_tb.sink.get_time_now().get_real_secs()
    ...
    logger.info("rx_freq: {} tx_freq: {}".format( options.rx_freq, options.tx_freq))
...
#    tx_tb = TxTopBlock(options)
    if IS_BS_ROLE:
        ...
        NODE_ID = rx_tb.source.get_usrp_mboard_serial()
...
#    logger.info("\nClock Rate: {} MHz".format(tx_tb.sink.get_clock_rate() / 1000000))
    ...
    pc_now = time.time()
    rx_tb.source.set_time_now(uhd.time_spec(pc_now))
...
#                
#        
#                (pktno,) = struct.unpack('!H',down_payload)
#                logger.info("get {}".format(pktno))
#                
#                up_payload = struct.pack('!H',(pktno+1) & 0xffff)
#                
#             
#                tx_tb.txpath.send_pkt(up_payload)
#                time.sleep(0.01)
#              
...
# 
#                count=0
#                while (False==upload_event.isSet()):
#                #if False == upload_event.wait(1):
#                    time.sleep(0.1)
#                    count=count+1
#                    if count>10:
